<%@ Page Language="C#" AutoEventWireup="true" CodeFile="member_game_setting.aspx.cs" Inherits="view_control_member_auth_setting" %>
<%@ Import Namespace="ClassLibrary" %>
<%@ Import Namespace="ClassLibrary.Define" %>
<%@ Import Namespace="ClassLibrary.GameDefine" %>
<%@ Import Namespace="Newtonsoft.Json.Linq" %>
<% 
    JToken jt_member = data["member_data"];
%>
...
<script>
    ...
    //var allbetfg = {
    //    allbetfgPercentHtml: function (data) {

    //        var allbetTypePercent = Math.floor(parseFloat(data['gameTypePercent']) * 100000) / 1000;
    //        var allbetTypePercentParent = Math.floor(parseFloat(data['gameTypePercentParent']) * 100000) / 1000;

    //        var html = '';
    //        html += '<td class="' + (data['isHidden'] ? 'isForceHidden': '') + '">';
    //        html += '<div class="input">';
    //        html += sportcfg.isLookSelf ? gameTypePercent:'<input id="betField_' + data['gameTypeId'] + '_' + sportcfg.betFieldMap.OWN_PERCENT + '" type="text" class="gameForm gameTypeField-js percent percent-js sportSetField-js" name="' + sportcfg.betFieldMap.OWN_PERCENT + '" data-field="' + sportcfg.betFieldMap.OWN_PERCENT + '" data-game-type-id="' + data['gameTypeId'] + '" data-section-id="" data-section-combine="" data-bet-combine="" data-bet-type-id=""  value="' + gameTypePercent + '" data-max-limit="' + gameTypePercentParent + '" />&nbsp;<=&nbsp;' + gameTypePercentParent;
    //        html += '</div>';
    //        html += '</td>';
    ...
    var walletTypesOpen = {
        <%=PointControl.Wallet.Define.WalletTypes.Sportlottery %>: {isOpen: data['x'], key: 'sport'},
        <%=PointControl.Wallet.Define.WalletTypes.Allbet %>: {isOpen: data['H'], key: 'allbet'},
        <%=PointControl.Wallet.Define.WalletTypes.UC8 %>: {isOpen: data['I'], key: 'uc8'},
        <%=PointControl.Wallet.Define.WalletTypes.Ultx %>: {isOpen: data['J'], key: 'ultx'},
        <%=PointControl.Wallet.Define.WalletTypes.Maya %>: {isOpen: data['K'], key: 'maya'},
        <%=PointControl.Wallet.Define.WalletTypes.QTech %>: {isOpen: data['J'], key: 'qtech'},
        <%=PointControl.Wallet.Define.WalletTypes.HowLottery %>: {isOpen: data['M'], key: 'howlottery'}
    }
...
<%--    <div class="mui-panel">
        ...
        <div class="infoPanel">  
            <div class="inline textCenter">
                <br />
                <div id="submit" class="mui-btn mui-btn--primary submitBtn update-js" data-disabled="false"><i class="icon-spinner"></i>儲存</div>
            </div>
...
<div class="mui-panel">
    <h1 class="formBoxTitle">會員賽事設定 - <span id="username"></span></h1>
